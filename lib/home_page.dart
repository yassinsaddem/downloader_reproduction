  ...
  void _bindBackgroundIsolate() {
    ...
    _port.listen((dynamic data) async {
      ...
      if (_tasks != null && _tasks!.isNotEmpty) {
        ...
        final tasksData = await FlutterDownloader.loadTasksWithRawQuery(query: 'SELECT * FROM task WHERE task_id = "$taskId"');
        if(tasksData != null && tasksData.isNotEmpty){
          final taskData = tasksData.first;
          log('Reproduction Log: file name is including file path ${taskData.savedDir}/${taskData.filename}');
          final file = File('${taskData.savedDir}/${taskData.filename}');
          if(file.existsSync()){
            log('Reproduction Log: file length should increment ${file.lengthSync()}');
          }
        }
        setState(() {
          task
        ...
        ..._items.map(
          (item) {
            ...
            return DownloadListItem(
              ...
              onActionTap: (task) {
                log('task action tap: ${task.status}');
